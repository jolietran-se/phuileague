    ...
    <div class="setting-section setting-main">
        <div class="container" id="content">
            @if ($tournament->tournament_type_id === 3)
                ...
                <div class="col-md-12" id="list-match-k">
                        ...
                        <div class="tab-k-item" id="roundK{{pow(2, $i)}}">
                            ...
                            <table class="table table-striped" style="border: 1px solid #326295;">
                                ...
                                @foreach ($matchsK as $match)
                                    ...
                                    @if (is_object($clubs->where('id',$match->clubA_id)->first()))
                                    ...
                                    @endif
                                    @if (is_object($clubs->where('id',$match->clubB_id)->first()))
                                    ...
                                    <tr class="show-detail show-detail-{{$match->id}}" id="show-match-{{$match->id}}" role="button"
                                        data-match-id="{{ $match->id }}"
                                                ...
                                                data-a-name="{{ isset($clubs->where('id', $match->clubA_id)->first()->name)?$clubs->where('id', $match->clubA_id)->first()->name:'Đội 1' }}"
                                                data-b-name="{{ isset($clubs->where('id', $match->clubB_id)->first()->name)?$clubs->where('id', $match->clubB_id)->first()->name:'Đội 2' }}"
                                                data-a-logo="{{ isset($clubs->where('id', $match->clubA_id)->first()->logo)?$clubs->where('id', $match->clubA_id)->first()->logo:'' }}"
                                                ...
                                                >
                                        <td style="width:20%"><small>#{{$indexk}} @php $indexk++ @endphp</small></td>
                                        <td style="width:25%" class="text-right">
                                            <small>{{ isset($clubs->where('id', $match->clubA_id)->first()->name)?$clubs->where('id', $match->clubA_id)->first()->name:"#Đội 1" }}</small>
                                        ...
                                        <td style="width:25%">
                                            ...
                                            @else 
                                                <img src="{{ asset('storage/club-logos/logo_default.jpg') }}" alt="" class="img-circle" style="width:30px">
                                            @endif
                                            <small>{{ isset($clubs->where('id', $match->clubB_id)->first()->name)?$clubs->where('id', $match->clubB_id)->first()->name:"#Đội 2" }}</small>
                                        </td>
